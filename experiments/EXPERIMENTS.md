...
| `research/performance-boost` | [experiments/performance-boost/results.md](performance-boost/results.md) | curated single-model lever board (accuracy↑, parallel to compression) + **E43** (weight EMA/SWA on AWP — discarded) · **E44** (granite→qwen3 backbone transfer of AWP/ELR) |
...
| E39 | word-aug | **Offline word-level augmentation** (user 2026-07-13) — generate NEW labeled rows by rewriting free-text (`current_prompt`+`USER:`; code identifiers protected). **B1 C-MLM** (XLM-RoBERTa-large mask→predict, mask_p 0.10 = EDA sweet spot) · **B2 LLM** (`gpt-4o-mini` via OpenAI **Batch API** ≈$6; local Qwen-14B-AWQ retired). **Baseline = E38 t070** (AWP-Optuna best, session-fold-0, fold-F1 **0.7806** REUSED as A0 — no retrain). **balanced** (lift all classes to max 8937, 69,118 copies → macro-F1 play) vs **blanket** (uniform 69,118, equal volume). Factorial A0+{B1,B2}×{bal,bln}=4 trainings | A0 = t070 0.7806 · macro-F1 (fold-0 14k holdout) | — | — | — | 🏃 **RUNNING (2026-07-13, vast 44747112 4×3090)** — C-MLM gen done ×2 seeds (backed up local `output/e39/`); **C-MLM arms training** (t070 recipe, inject verified train=125,118/val=14,000, leak-free); **LLM batches submitted** (3×≤50k, awaiting). QC ✅: C-MLM 100% struct/label preserved + word-drift on entities; GPT-4o-mini 8/8 JSON, identifiers verbatim. Lit prior: word-aug vanishes at high-resource → **balanced = the bet**. [word_aug/results.md](word_aug/results.md) |
...
| E44 | performance-boost | **granite→qwen3 backbone transfer of the single-model levers** — do AWP / ELR carry to qwen3-0.6B (E8b+LS, the LB-stronger backbone)? qwen3 2×2 on richargs·full_data·ep4: LS+AWP (finetune.py, best-epoch) · LS+ELR · LS+AWP+ELR (elr.py, last-epoch). Within-qwen3 deltas + finals vs granite equivalents | qwen3_ls E8b+LS **LB 0.77921** / slice; granite equiv: AWP 0.7822 · ELR+LS 0.7704 · AWP+ELR 0.7806 | 🏃 **partial**: LS+ELR **done 0.7763**@ep4 · LS+AWP ep2 0.7638 (running) · LS+AWP+ELR ep2 0.7646 (running). Within-qwen3 pattern mirrors granite (**AWP≫ELR**, ELR≈redundant on AWP); slice ~0.01 below granite but qwen3 won LB in E8 → finals+LB judge | — | — | 🏃 **RUNNING** (2026-07-13, vast 44742238 GPU1/2/3). [performance-boost/results.md](performance-boost/results.md) |
