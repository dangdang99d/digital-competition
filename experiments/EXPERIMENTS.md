...
| E8 | combine | **Max-performance combo** (backbone × richargs × full-data [× LS]) | **LB SOTA 0.77427** | 🏆 **E8a+LS granite 0.7803** (LS +0.0097 vs E8a 0.7706; **+0.0060 ABOVE SOTA on CV**) · E8b+LS qwen3 0.7659 (LS +0.0016, weak on qwen3) · E8a 0.7706 · E8b 0.7643 · CV≠LB | [e8a_vs_champion](combine/figures/e8a_vs_champion_slice.png) · [e8a_ls_transfer](combine/figures/e8a_ls_transfer.png) | ✅ **`submit_0707_granite_ls.zip` BUILT+VERIFIED** (val-F1 reproduced 0.7799, parity 5/5, no logit_bias, richargs; ⚠ pins transformers==4.51.3) — READY TO UPLOAD · **`submit_0707_qwen3_ls.zip` BUILT** (qwen3 hedge, richargs, no logit_bias, same prune pipeline; ⚠ supervisor smoke-test inconclusive — local `test.jsonl` is only a 5-record sample — **user: quick-verify before upload**) | ✅ **E8a+LS granite = best (upload-ready primary)** · qwen3 = LB hedge · **TOP PRIORITY** |
...
| E16b | pruning | compress the BEST model — depth-prune granite E8a+LS 0.7803 (keep-11 then keep-14 + recovery) | E8a+LS 0.7803 | ❌ **NO-GO**: keep-11/lr5e-6 → **0.1602**; keep-14/lr2e-5/3ep → **0.2365** (both near-random). granite/ModernBERT **doesn't tolerate aggressive depth-prune** — layers less redundant than qwen3's, recovery can't rebuild the halved encoder | — | — | ❌ **DONE — NO-GO** (2 attempts cratered; backbone-dependent: qwen3 depth-prune ~free (E16), granite does not recover. Best model E8a+LS is already fast at 5:10 — no compression needed) |
