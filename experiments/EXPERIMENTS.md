...
| `research/augmentation` (revisit) | [experiments/augmentation/results.md](augmentation/results.md) | E36 (data-aug revisit: A inference multi-view averaging — metadata-views/progressive-history/length-padding all ❌; A2 train-mixed-views · C mixup · D E15c open) |
...
| E37 | pooling | **Granite classifier sequence pooling** (user 2026-07-13: "use every token, like an LSTM?") — one axis vs champion, from-scratch: **P0 cls** (control = current `[CLS]`/first-token) · **P1 mean** (masked mean, native ModernBERT) · **P2 attn** (learned attention pool — the transformer-native "use all tokens") · **P3 mean+max** (opt). LSTM form rejected (redundant with bidirectional self-attention + T4 cost). granite/ModernBERT only | champion cls anchor (from-scratch) · full_data CV **0.7803** context | — | — | — | 🟢 **CODE READY + smoke-passed (2026-07-13)** — `experiments/pooling/{pooling.py,run_e37.py}`, all in the E37 folder, `src/` untouched; `run_e37.py` injects pooling in-process + reuses `finetune.main()` (champion recipe, bs16×1). Smoke ✅ all 3 modes (attn trains/evals, weights save). **Awaits vast GO** (`python -m experiments.pooling.run_e37 --pooling cls|mean|attn`). Prior modest (ceiling is label-ambiguity not capacity: E13/E33/reasoning), but cheap + untested; mean may align w/ granite's embedding-pretraining pooling. [pooling/results.md](pooling/results.md) |
