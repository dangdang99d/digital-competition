...
| `research/compression` | [experiments/compression/results.md](compression/results.md) | **E31 (granite) + E40 (qwen3)** structured-compression combination searches, shared method board (size↓ vs speed↑). Structured only. granite: winning combo → optimal model post-E30/E29. qwen3: depth×FFN×attn factorial, both axes proven (E4/E16), compression = shippability enabler. Absorbs E4/E16/E16b/E18 |
...
| `research/augmentation` (revisit) | [experiments/augmentation/results.md](augmentation/results.md) · [E38](augmentation/results_e38.md) | E36 (data-aug revisit: A inference multi-view averaging — metadata-views/progressive-history/length-padding all ❌; A2 train-mixed-views · C mixup · D E15c open) · E38 (training-time char-level noise on free-text only) |
...
| E38 | augmentation | **Training-time char-level text augmentation (free-text only)** (user 2026-07-13) — char noise (swap/delete/insert/substitute, per-char prob) applied dynamically per-epoch to `current_prompt`+`USER:` content ONLY (structure/action-names/args/metadata/labels byte-exact); eval clean. Surface-robustness regularizer, distinct from E24 (token drop) & E32 (embedding noise). Char chosen over synonym (bilingual KO/EN + code-heavy → language-agnostic wins; synonym deferred, contextual dropped). **Language-aware noiser**: Korean = JAMO-level (decompose→perturb 1 component→recompose; vowel/batchim/tense-consonant typos), English = ASCII char ops. Two knobs p(per-char)×α(clean+noised mix rate); arms A0 control / A1 p.02×α.5 (primary) / A2 p.05×α.5 / A3 p.02×α1.0 | champion cls anchor (from-scratch) · full_data CV **0.7803** context | — | — | — | 🟢 **CODE READY + smoke-passed (2026-07-13)** — `experiments/augmentation/{text_noise.py,run_e38.py}`, in the E38 folder, `src/` untouched; run_e38 replaces `build_dynamic_dataset` in-process (train noised, val clean). Smoke ✅ (KO jamo + EN char typos verified; structure byte-identical). **Awaits vast GO** (`python -m experiments.augmentation.run_e38 --p <p> --alpha <a>`). Lit: char-noise aug = robustness gain, ~NEUTRAL on clean test (2110.07428/Belinkov&Bisk); jamo validated for KO (1707.06341); classification/high-resource = low headroom → tight sweep + hard gate. Venue = vast. [augmentation/results_e38.md](augmentation/results_e38.md) |

| E39 | word-aug | **Offline word-level augmentation** (user 2026-07-13) — generate NEW labeled rows by rewriting free-text (`current_prompt`+`USER:`; code identifiers protected). **B1 C-MLM** (XLM-RoBERTa-large mask→predict) · **B2 LLM** (Qwen2.5-14B-Instruct-AWQ paraphrase, vLLM). **56k/14k baseline** (honest 14k val, not 3.5k), A0 control = 56k no-aug. **balanced** (lift all classes to max 8937, 69,118 copies → macro-F1 play) vs **blanket** (uniform 69,118, equal volume). Factorial A0+{B1,B2}×{bal,bln}=5 runs | A0 56k no-aug control · macro-F1 (14k val) | — | — | — | 🟢 **GENERATING (2026-07-13, vast 44747112 4×3090)** — code ready+verified (`experiments/word_aug/`, src/ untouched; pool 56k, bal/bln both 69,118 copies, post-balance flat 8937). C-MLM gen (main venv) + LLM gen (vLLM isolated venv). Lit prior: word-aug vanishes at high-resource → **balanced = the bet** (rare-class F1), blanket likely flat. [word_aug/results.md](word_aug/results.md) |
| E40 | compression | **Structured-compression combination SEARCH (qwen3)** — the qwen3 mirror of E31, higher headroom (compute stack = 74% of params: FFN 44% / attn 30%, vs granite's 35%). Both axes already PROVEN individually but never combined: FFN-factor r512+recovery **0.7688 net-+0.0045** (E4), depth 28→14+recovery **0.7638 ~free ~2×** (E16). Stage-B factorial: **depth × FFN-lowrank × attn-SVD** (NEW: attn is 30% here), prunes composed then ONE joint recovery per combo → Pareto (F1, ms/sample, params). Motivation: qwen3 had the accuracy edge (LB **0.77921** > granite 0.77738 @E8) but 9:18/30k = un-shippable; compression is the ENABLER + could re-enter it as a 4th ensemble member. Structured only | qwen3_ls **0.77921** LB (must hold) · E8b 0.7643 anchor · budget 10min/30k | — | — | — | 🔲 **QUEUED** (user 2026-07-14) — priors exist (BI selection + `factored_ffn.py` + SVD-LLM whitening all validated on qwen3); new code = attn-SVD modules. Board: [compression/results.md](compression/results.md) |
...
### E40 · Structured-compression combination SEARCH on qwen3 (user 2026-07-14) — 🔲 QUEUED
...
- **Both axes already PROVEN individually on qwen3 (never combined — that product is the bet):**
  ...
  - E4's note claims these STACK — untested. E40 tests the product.
