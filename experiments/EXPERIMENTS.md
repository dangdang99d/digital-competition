...
| E22 | coreset | **Drop-noisy data selection** — retrain granite on cleaned subsets (drop mislabeled/unlearnable), maximize macro-F1. Scorers: C1 cleanlab (k-fold OOF) · C2 AUM · C4 cartography · C5 forgetting · C6 EL2N · C7 PVI → `--keep_indices` retrain | granite CE full-56k = **0.7458** | 🏃 §1 suitability probe: ~**6% two-model consensus mislabels** (removable), but **40% of errors low-conf ambiguity** + errors on synonymous file-ops (irreducible) → 🟡 **CONDITIONAL GO — surgical only**; blanket "drop all wrong" predicted to LOWER macro-F1 (starves rare classes) | [datamap](coreset/figures/datamap.png) · [suitability](coreset/figures/suitability_error_profile.png) | n/a | 🏃 **RUNNING** — plumbing (`--keep_indices`/`--log_dynamics`) + 6 scorers implemented & smoke-tested; baseline + cleanlab k-fold + scorers launched on GPUs 1/2/3. Decisive read = C1+C4; every retrain reports per-class F1 (rare-class guard). Group 3 gated on a proven denoise gain |
...
| E24 | token-selection | **Upstream feature/token selection** (a *filter*: score importance once → retrain once) — **A** attention/saliency token-select + top-k refit (k∈{90,80,70,60}%) · **B** field-level occlusion selection. **C (rationale) + D (controls) → Deferred.** | granite champion **E8a+LS richargs 0.7803** (`submit_0707_granite_ls.zip`, 3.5k held-out) | — | — | — | 🟢 READY (user 2026-07-09) — A+B built+validated (`experiments/token-selection/`); Phase-0 training-free; decoupled from LTP/E18. Seq: A → B. Design in [token-selection/results.md](token-selection/results.md) |
...
### E22 · Coreset — drop-noisy data selection — 🟡 CONDITIONAL GO (surgical), ✋ AWAITING USER GO — 3 GPUs, ~12-14 GPU-h
...
### E24 · Token/feature selection — **A (token) + B (field)** — 🟢 READY — branch: **token-selection** — design in [experiments/token-selection/results.md](token-selection/results.md)
