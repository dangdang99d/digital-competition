"""F1-gate the PRUNED int8 ONNX (remapped ids): build engine on this GPU, score the 3.5k val slice."""
...
d = np.load(f'{W}/val_tokens_pruned.npz')
...
assert p.parse(open(f'{W}/model_int8_pruned.onnx', 'rb').read()), 'parse failed'
