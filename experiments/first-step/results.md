...
| Exp (arm) | Model | Status | Result | Δ vs generalist 0.555 | Verdict |
|---|---|:--:|---|---|---|
| E1 · `--zero_history` (7k real first-steps) | bge-m3 | ✅ | 0.4283 | −0.127 | specializing HURTS |
| E1 · `--strip_history` (all 56k, history stripped) | bge-m3 | ✅ | 0.4407 | −0.114 | specializing HURTS |

## E1 — first-step specialist ceiling (A/B)
- **Model:** bge-m3 (both arms; the reference generalists are bge-m3 0.555 and qwen3 0.573 on the same slice).
...
## Notes
- step==1 ⟺ len(history)==0 exactly (9,000/70,000). First-step error ~2× mid-session (36.7% vs ~18%); models prior-collapse onto list_directory/plan_task. Bigger model (qwen3 vs bge-m3) barely helps (+0.018).
